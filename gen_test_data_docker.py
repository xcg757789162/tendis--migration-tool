#!/usr/bin/env python3
"""
生成 1GB 测试数据到 Tendis 集群（在 Docker 容器内执行）
- 80% String 类型
...
def connect_cluster():
    ...
    try:
        ...
        rc.ping()
        print("OK: Connected to Tendis cluster")
        return rc
...
def write_big_keys(rc):
    """写入大 Key"""
    print(f"\nWriting {BIG_KEY_COUNT} big keys...")
    total_size = 0
    ...
    print(f"Big keys done, total: {total_size/1024/1024:.2f} MB")
    return total_size


def write_normal_keys(rc, remaining_size):
    """写入普通 Key"""
    print(f"\nWriting normal keys, target: {remaining_size/1024/1024:.2f} MB")
    ...
    while total_written < remaining_size:
        ...
        try:
            ...
            if current_time - last_report_time >= 10:
                ...
                print(f"  Progress: {progress:.1f}% | Written: {total_written/1024/1024:.2f} MB | "
                      f"Speed: {speed:.2f} MB/s | Keys: {sum(key_counts.values())}")
                last_report_time = current_time
                
        except Exception as e:
            print(f"  WARN: {key_name}: {e}")
            time.sleep(0.1)
    
    elapsed = time.time() - start_time
    print(f"\nNormal keys done")
    print(f"   Total: {total_written/1024/1024:.2f} MB")
    print(f"   Time: {elapsed:.1f}s")
    print(f"   Speed: {total_written/elapsed/1024/1024:.2f} MB/s")
...
def get_cluster_stats():
    ...
    for node in NODE_CONFIGS:
        try:
            ...
            print(f"   Node {node['port']}: {dbsize} keys, {memory_used/1024/1024:.2f} MB")
        except Exception as e:
            print(f"   Node {node['port']}: ERROR - {e}")
    
    print(f"   Total: {total_keys} keys, {total_memory/1024/1024:.2f} MB")
    return total_keys, total_memory


def main():
    print("=" * 60)
    print("Tendis Cluster Test Data Generator")
    print("=" * 60)
    print(f"Target: {TARGET_SIZE_GB} GB")
    print(f"Distribution: String {STRING_RATIO*100}%, Hash {HASH_RATIO*100}%, "
          f"List {LIST_RATIO*100}%, Set {SET_RATIO*100}%, ZSet {ZSET_RATIO*100}%")
    print(f"Big keys: {BIG_KEY_COUNT} ({BIG_KEY_MIN_SIZE/1024/1024}-{BIG_KEY_MAX_SIZE/1024/1024} MB)")
